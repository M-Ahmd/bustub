...
auto BufferPoolManager::NewPage() -> page_id_t {
  ...
  // pin_count stays 0 — CheckedWritePage/CheckedReadPage will pin it.

  page_table_[new_page_id] = selected_frame_id;

  // Register with replacer so it knows about this frame, but don't pin yet.
  replacer_->RecordAccess(selected_frame_id, new_page_id);
  replacer_->SetEvictable(selected_frame_id, true);
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  ...
  if (it == page_table_.end()) {
    // Page not in buffer pool — treat as success.
    return true;
  }
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  std::shared_ptr<FrameHeader> frame;
  {
    ...
    auto it = page_table_.find(page_id);
    if (it != page_table_.end()) {
      // Page already in memory — just pin it.
      frame = frames_[it->second];
      frame->pin_count_.fetch_add(1);
      replacer_->RecordAccess(it->second, page_id, access_type);
      replacer_->SetEvictable(it->second, false);
    } else {
      // Need a frame: free list or eviction.
      frame_id_t frame_id;
      if (!free_frames_.empty()) {
        ...
        auto &victim = frames_[frame_id];
        if (victim->is_dirty_) {
          FlushPageUnsafe(victim->page_id_);
        }
  ...
  // BPM latch released — guard constructor acquires rwlatch.
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  ...
  // Take a shared lock on the page for a consistent read of the data.
